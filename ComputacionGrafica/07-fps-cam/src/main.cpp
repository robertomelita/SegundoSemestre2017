...
int main(){
    ...
    int N = 100;
/*    if(argc != 2){
        fprintf("Error ./bin/prog N");
        return EXIT_FAILURE;
    } */
    srand(1);
/*    float pose1[] ={0,0,0};
    float pose2[] ={0,-2.5f,0};
    float pose3[] ={-4.0f,0,-4.0f};
    float pose4[] ={4.0f,0,-4.0f};

    malla *e1 = new malla((char*)"mallas/suzanne.obj", pose1);
    malla *e2 = new malla((char*)"mallas/plano.obj", pose2);
    malla *e3 = new malla((char*)"mallas/muro.obj", pose3);
    malla *e4 = new malla((char*)"mallas/muro.obj", pose4);*/

    malla **monos = (malla**)malloc(sizeof(malla*)*N);
    for(int i = 0; i < N; i++){
        monos[i]->setpos(glm::vec3(10.0f*(float)rand()/(float)RAND_MAX,10.0f*(float)rand()/(float)RAND_MAX,10.0f*(float)rand()/(float)RAND_MAX));
        monos[i] = new malla((char*)"mallas/suzanne.obj");
        monos[i]->setmatloc(shader_programme,(char*)"mallas/suzanne.obj");
    }
    ...
    while (!glfwWindowShouldClose(g_window)){
        ...
        //pared2
        glBindVertexArray(e4->getvao());
        glDrawArrays(GL_TRIANGLES,0,e4->getnumvertices()); */
